...
- Windows enable is idempotent for an exact v4 definition, upgrades only recognized prior Lifeline definitions with the stable URI/author, exactly one same-current-user logon trigger/principal, canonical root, and one expected stable action, and rejects extra triggers/principals, a different user/root, or a foreign/multi-action same-name task without overwrite. Safe owned settings drift—including unknown extra settings—is repaired to the structurally exact contract. Concurrent absent-state or owned-drift enables use closed, invocation-unique scheduler XML files and converge when a losing create reads the exact expected task. After a failed owned-drift create, exact prior XML is reported unchanged, while missing, non-exact, foreign, or ambiguous readback is preserved and fails closed without stale rollback authority. Only a successful owned scheduler mutation followed by verified absence may restore the exact prior XML through a separate invocation-owned definition. Disable persists disabled intent only after an exact post-delete query proves absence.
