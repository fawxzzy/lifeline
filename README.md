...
- The Windows task action runs a byte-verified, content-addressed Lifeline `dist` snapshot beneath `<runtime-home>/.lifeline/startup/windows/`, passes the runtime home explicitly with `--root`, and never depends on the enabling source worktree after registration. Re-enable repairs corrupt or incomplete snapshot contents even when launcher metadata still claims the expected hash.
- Windows enable is idempotent for an exact v4 definition, upgrades only recognized prior Lifeline definitions with the stable URI/author, same current-user trigger/principal, canonical root, and one expected stable action, and rejects a different-user, different-root, foreign-action, or multi-action same-name task without overwrite. Safe owned settings drift—including unknown extra settings—is repaired to the structurally exact contract. A failed new registration is removed, while a failed owned upgrade restores and verifies its exact prior definition. Disable applies the same ownership proof before removal.
