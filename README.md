...
- The Windows task action runs a byte-verified, content-addressed Lifeline `dist` snapshot beneath `<runtime-home>/.lifeline/startup/windows/`, passes the runtime home explicitly with `--root`, and never depends on the enabling source worktree after registration. Snapshot materialization is cross-process safe: each publisher builds a unique staging directory, rehashes its payload against the planned content address, writes metadata last, and uses a filesystem lease for atomic directory publication; a losing publisher accepts only the winner's exact bytes. Invalid snapshots are quarantined before atomic replacement, while dead publisher leases are recoverable without exposing partial canonical content. Re-enable repairs corrupt or incomplete snapshot contents even when launcher metadata still claims the expected hash. Roots equal to or nested beneath the active `dist` source, and any other source/destination overlap, fail before hashing or copying.
- Windows enable is idempotent for an exact v4 definition, upgrades only recognized prior Lifeline definitions with the stable URI/author, exactly one same-current-user logon trigger/principal, canonical root, and one expected stable action, and rejects extra triggers/principals, a different user/root, or a foreign/multi-action same-name task without overwrite. Safe owned settings drift—including unknown extra settings—is repaired to the structurally exact contract. Concurrent absent-state or owned-drift enables use closed, invocation-unique scheduler XML files and converge when a losing create reads the exact expected task. Lifeline re-reads exact-installed tasks after launcher verification/repair and byte-compares inspected owned XML immediately before `/Create /F` or `/Delete`; changed ownership is preserved and fails closed. After a failed owned-drift create, exact prior XML is reported unchanged, while missing, non-exact, foreign, or ambiguous readback is preserved without stale rollback authority. Only a successful owned scheduler mutation followed by verified absence may restore the exact prior XML through a non-forced, separately guarded invocation-owned definition. Disable persists disabled intent only after an exact post-delete query proves absence.
- The task uses `lifeline restore --startup`: ordinary `lifeline restore` still skips intentionally stopped apps, while startup mode may revive a stopped app only when it remains `restorable: true`. The startup wrapper correlates every restored supervisor with its persisted identity while holding the process tree. A partial multi-app launch failure or the first blocked, crash-loop, unhealthy, missing, mismatched, or stale-dead supervisor state cleans every supervisor started by that invocation before failing. Cleanup authority is PID-owned: the expected supervisor comparison and stopped/blocked transition share the runtime state store's cross-process mutation lease, so graceful app down and fresh `stopped` verification apply only while persisted identity still matches; otherwise Lifeline proves the captured original PID dead while preserving and re-verifying the newer replacement supervisor state. Clean `lifeline down` establishes the ordinary terminal state and lets the wrapper exit successfully.
