...
- Windows enable is idempotent for an exact v4 definition, upgrades only recognized prior Lifeline definitions with the stable URI/author, exactly one same-current-user logon trigger/principal, canonical root, and one expected stable action, and rejects extra triggers/principals, a different user/root, or a foreign/multi-action same-name task without overwrite. Safe owned settings drift—including unknown extra settings—is repaired to the structurally exact contract. Concurrent absent-state enables use closed, invocation-unique scheduler XML files and converge when a losing create reads the exact expected task; verified absence is a create failure, and non-exact or ambiguous readback is preserved as installed/unknown without deletion. Owned-upgrade rollback uses the same invocation-owned XML rule, must retain or restore exact prior XML, and reports that registration as still physically installed. Disable persists disabled intent only after an exact post-delete query proves absence.
- The task uses `lifeline restore --startup`: ordinary `lifeline restore` still skips intentionally stopped apps, while startup mode may revive a stopped app only when it remains `restorable: true`. The startup wrapper correlates every restored supervisor with its persisted identity while holding the process tree. A partial multi-app launch failure or the first blocked, crash-loop, unhealthy, missing, mismatched, or stale-dead supervisor state cleans every supervisor started by that invocation before failing. Cleanup authority is PID-owned: graceful app down and fresh `stopped` verification apply only while persisted identity still matches; otherwise Lifeline proves the captured original PID dead while preserving and re-verifying the newer replacement supervisor state. Clean `lifeline down` establishes the ordinary terminal state and lets the wrapper exit successfully.
