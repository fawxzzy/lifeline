...
test("exports only the two honest Lifeline intake candidates", () => {
  const output = buildProjectBoardOwnerExport(roadmap, roadmapBytes);
  assert.equal(roadmap.workItems.length, 10);
  assert.deepEqual(output.cards.map((card) => card.record.card_id), ["LIF-201", "LIF-202"]);
  assert.ok(output.cards.every((card) => card.record_status === "candidate"));
  assert.ok(output.cards.every((card) => card.record.lifecycle === "intake"));
...
test("rejects duplicate identities and invented priority", () => {
  ...
  prioritized.workItems.find((item) => item.id === "LIF-201").priority = "high";
