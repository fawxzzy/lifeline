...
async function runLifeline(cwd, ...args) {
  ...
  if (stderr.trim().length > 0) {
    throw new Error(
      `Unexpected stderr for startup ${args.join(" ")}: ${stderr.trim()}`,
    );
  }
  return stdout;
}
...
async function cleanupOwnedWindowsTask(cwd) {
  ...
  if (!taskXml) return;
  const ownershipMarker = `Managed by Lifeline Windows startup v4. Root: ${cwd}`;
  assert(
...
async function main() {
  const tempDir = await mkdtemp(
    path.join(os.tmpdir(), "lifeline-startup-roundtrip-"),
  );
  assert(
  ...
  try {
    ...
    assert(
      enableOutput.includes("Startup intent enabled."),
      "Expected startup enable confirmation.",
    );

    const enabledState = await readStartupState(tempDir);
    assert(
      enabledState.intent === "enabled",
      `Expected enabled intent, got ${enabledState.intent}`,
    );
    assert(
    ...
    const backendStatus = readStatusLine(
      statusOutput,
    ...
    assert(
      mechanism.length > 0,
      "Expected startup status to report non-empty mechanism line.",
    );

    const disableOutput = await runLifeline(tempDir, "startup", "disable");
    assert(
      disableOutput.includes("Startup intent disabled."),
      "Expected startup disable confirmation.",
    );

    const disabledState = await readStartupState(tempDir);
    assert(
      disabledState.intent === "disabled",
      `Expected disabled intent, got ${disabledState.intent}`,
    );

    const statePath = path.join(tempDir, ".lifeline", "startup.json");
    await writeFile(statePath, '{"intent":', "utf8");
    const recoveredStatusOutput = await runLifeline(
      tempDir,
      "startup",
      "status",
    );
    ...
    console.log(
      "Deterministic startup roundtrip verification passed (enable/status/disable).",
    );
  } finally {
...
main().catch((error) => {
  ...
  console.error(
    `Deterministic startup roundtrip verification failed: ${message}`,
  );
  process.exitCode = 1;
});
