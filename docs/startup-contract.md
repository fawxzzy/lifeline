...
- The action executes the active Node binary plus a byte-verified, content-addressed Lifeline `dist` snapshot beneath `<runtime-home>/.lifeline/startup/windows/`, passes `--root <runtime-home>` explicitly with Windows command-line escaping (including doubled trailing backslashes before a closing quote), sets the working directory to that runtime home, and invokes `restore --startup`. Non-dry-run re-enable verifies the full expected file set and bytes, repairing a corrupt snapshot without trusting metadata alone. Cross-process materialization builds a unique staging directory, rehashes its canonical payload against the previously planned content address, writes metadata last, then uses a bounded filesystem lease and atomic directory publication; a losing publisher accepts only an exact winner. Invalid snapshots are quarantined before atomic replacement, while dead publisher leases are recoverable without exposing partial canonical content. Re-enable repairs corrupt or incomplete snapshot contents even when launcher metadata still claims the expected hash. Roots equal to or nested beneath the active `dist` source, and any other source/destination overlap, fail before hashing or copying.
...
- `startup enable` may upgrade a recognized prior Lifeline definition only when its stable URI/author, single current-user trigger/principal (SID or Scheduler-canonicalized account), canonical root, and exactly one versioned stable `Exec` action all match the ownership contract. A second trigger, principal, or action is foreign and blocks overwrite/removal; safe settings drift repairs to the exact v4 structure. Exact-installed tasks receive a final exact readback after launcher verification or repair. The exact inspected owned XML is also queried and byte-compared again immediately before any `/Create /F` or `/Delete`; a changed or ambiguous task is preserved without mutation, while an exact concurrent v4 winner converges. An owned-drift create failure reports exact prior XML as unchanged failure and preserves missing, different, foreign, or ambiguous readback without stale rollback authority. Only a successful owned scheduler mutation followed by verified absence may restore and verify the exact prior XML through a separately guarded, non-forced invocation-owned definition. An unverifiable transaction is a blocker, and any observed task reports physically installed. `startup disable` uses the same ownership proof and persists disabled intent only after post-delete query proves absence.
- A pre-v2 name/action-only task such as `LifelineRestoreAtLogon` running ambient `lifeline restore` has no stable Lifeline version/author/URI contract, explicit `--root`, or content-addressed launcher. Lifeline therefore classifies it as foreign/conflicting and will neither adopt, overwrite, nor remove it automatically, even when its name resembles an older Lifeline registration. Recovery is an explicit operator migration: first inspect the exact XML with `schtasks /Query /TN LifelineRestoreAtLogon /XML`, independently establish ownership and intended root, then manually remove that legacy task through Task Scheduler before running `lifeline startup enable`. A future legacy-adoption command would require separate authorization and an equally explicit ownership proof; ordinary enable/disable must remain fail-closed.
